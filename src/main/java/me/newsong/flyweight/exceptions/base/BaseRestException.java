package me.newsong.flyweight.exceptions.base;
...
import me.newsong.flyweight.exceptions.domain.RestFieldError;
...
public class BaseRestException extends RuntimeException {
	...
	private Locale defaultLocale = new Locale("zh", "CN");
	...
	public BaseRestException(HttpStatus status, int code, Locale locale, String field, Object rejectedValue) {
		this.status = status;
		this.code = code;
		this.error = Arrays.asList(new RestFieldError(field, rejectedValue, InternationalizeUtil
				.getMessage("i18n." + this.getMessageKey(), locale != null ? locale : defaultLocale)));
	}
